...
const FILE_LIKE_EXTENSIONS = new Set([
  "avif", "bmp", "c", "cc", "cpp", "css", "csv", "edf", "gif", "go", "h", "hpp", "htm", "html",
  "ipynb", "jpeg", "jpg", "js", "json", "jsonl", "jsx", "log", "m4a", "md", "markdown", "mat", "mjs",
  "mov", "mp3", "mp4", "npy", "npz", "pdf", "png", "py", "r", "rs", "sh", "svg", "tex", "toml", "ts",
  "tsv", "tsx", "txt", "wav", "webm", "webp", "xml", "yaml", "yml", "zip",
]);

const EXTERNAL_SCHEME = /^(?:https?|mailto|data|blob|javascript):/i;
const DOMAIN_LIKE = /^(?:www\.)?[\w.-]+\.(?:com|org|net|gov|edu|io|dev)(?:[/:?#]|$)/i;
const FILE_TAG = /<file\b[^>]*\bname\s*=\s*(["'])(.*?)\1/giu;
const MARKDOWN_TARGET = /!?\[[^\]]*\]\(\s*(?:<([^>]+)>|([^\s)]+))/gu;
const OSC8_TARGET = /\u001b\]8;[^;]*;([^\u0007\u001b]+)(?:\u0007|\u001b\\)/gu;
const INLINE_CODE = /(?<!`)`([^`\n]+)`(?!`)/gu;
const EXPLICIT_PATH = /(?:^|[\s([])((?:file:\/\/)?\/(?:[^\s`<>()[\]{}"']+\/)*[^\s`<>()[\]{}"']+|(?:\.\.?\/|~\/)[^\s`<>()[\]{}"']+)/gmu;
const AT_PATH = /(?:^|\s)@((?:\.\.?\/)?[^\s`<>()[\]{}"']+)/gmu;

export type ResourceReferenceSource = "tool" | "link" | "attachment" | "mention" | "embedded";
...
export function isLocalResourceReference(value: string): boolean {
  ...
  if (!reference || EXTERNAL_SCHEME.test(reference) || DOMAIN_LIKE.test(reference)) return false;
  if (/^(?:pi-embedded:\/\/|file:\/\/|vscode:\/\/file\/|\/|~\/|\.\.?\/)/i.test(reference)) return true;
  const withoutLocation = reference.replace(/[?#].*$/u, "").replace(/:\d+(?::\d+)?$/, "");
  const extension = /\.([A-Za-z0-9]{1,12})$/.exec(withoutLocation)?.[1]?.toLowerCase();
...
function contentParts(message: Record<string, unknown>): Array<Record<string, unknown>> {
  ...
  if (Array.isArray(content)) return content.filter((item): item is Record<string, unknown> => Boolean(item && typeof item === "object"));
...
function textReferences(text: string): Array<{ reference: string; source: ResourceReferenceSource }> {
  ...
  for (const match of text.matchAll(FILE_TAG)) add(match[2] ?? "", "attachment");
  for (const match of text.matchAll(MARKDOWN_TARGET)) add(match[1] ?? match[2] ?? "", "link");
  for (const match of text.matchAll(OSC8_TARGET)) add(match[1] ?? "", "link");
  for (const match of text.matchAll(INLINE_CODE)) add(match[1] ?? "", "mention");
  for (const match of text.matchAll(EXPLICIT_PATH)) add(match[1] ?? "", "mention");
  ...
  for (let messageIndex = messages.length - 1; messageIndex >= 0 && resources.length < limit; messageIndex -= 1) {
    ...
    for (let partIndex = parts.length - 1; partIndex >= 0 && resources.length < limit; partIndex -= 1) {
      ...
      if (part.type === "toolCall" && part.arguments && typeof part.arguments === "object") {
        ...
        for (const [key, value] of Object.entries(part.arguments as Record<string, unknown>)) {
          ...
          for (const rawReference of valuesForArgument(value)) {
            ...
            add({ key: `file:${referenceKey(reference)}`, reference, label: reference, source: "tool", toolName });
          }
        }
      }
    ...
    if (message.details && typeof message.details === "object") {
      for (const [key, value] of Object.entries(message.details as Record<string, unknown>)) {
        if (!isToolResourceArgumentKey(key)) continue;
        for (const rawReference of valuesForArgument(value)) {
          const reference = trimReference(rawReference);
          if (!isLocalResourceReference(reference)) continue;
          add({ key: `file:${referenceKey(reference)}`, reference, label: reference, source: "tool" });
        }
      }
    }
  }
