...
- The composer accepts multiline text. Desktop Enter submits and Shift+Enter inserts a line break; on a touch-first device the software keyboard's Return key inserts a line break and the explicit send action submits, while Ctrl/Command+Enter from an attached keyboard remains available. Each session keeps its own unsent draft, staged attachments, and project-file references, restored when the user switches back (in-memory — staged work does not survive a reload). Empty session partitions are discarded, while non-empty partitions remain until their user-owned work is sent or removed. `ComposerController` owns the bounded per-session attachment/project-file partitions and delivery lifecycle through `AppStore`'s narrow facade; `AppStore` remains the sole browser snapshot and cross-domain commit authority.
...
- Input submitted while Pi is active is explicitly sent as steering input or queued as a follow-up. `running`, `retrying`, `compacting`, and `queued` are one shared browser/host busy-state authority, so queued work exposes a visible two-option equal-width `Steer` / `Queue` segmented control beside abort; the selected mode changes the input placeholder and the send action's accessible name, rather than hiding delivery behind a shortcut. The entire Composer surface carries its slow theme-colored breathing halo only while this authority reports `running`, including while a descendant control retains focus; retrying and compacting retain their static semantic halos, and reduced-motion retains a static running halo. Conflict recovery remains abortable but is not part of active busy ownership. The activity surface reports the combined count as `N pending`, not `queued`; complete host-projected steering and follow-up arrays remain in one labelled Pending surface that preserves each array's order, supports multiple entries, marks rows `S` or `Q`, copies an individual row verbatim, and copies the displayed set as one numbered plain-text list without delivery marks. Pi exposes no browser RPC for editing or withdrawing committed queue entries, so the workbench does not pretend to manage them.
