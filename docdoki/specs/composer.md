...
- Input submitted while Pi is active is explicitly sent as steering input or queued as a follow-up. `running`, `retrying`, `compacting`, and `queued` are one shared browser/host busy-state authority, so queued work exposes a visible two-option equal-width `Steer` / `Queue` segmented control beside abort; the selected mode changes the input placeholder and the send action's accessible name, rather than hiding delivery behind a shortcut. Conflict recovery remains abortable but is not part of active busy ownership. The activity surface reports the combined count as `N pending`, not `queued`; complete host-projected steering and follow-up arrays remain in one labelled Pending surface that preserves each array's order, supports multiple entries, marks rows `S` or `Q`, copies an individual row verbatim, and copies the displayed set as one numbered plain-text list without delivery marks. Pi exposes no browser RPC for editing or withdrawing committed queue entries, so the workbench does not pretend to manage them.
