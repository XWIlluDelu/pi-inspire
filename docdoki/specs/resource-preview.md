...
- Content revalidation shares the project index's five-second freshness window and in-flight scans rather than invalidating the full index on every preview. Ignore-rule changes may therefore take effect after this short cache window; explicit Files refresh revokes stale index authority immediately for subsequent reads. Missing indexed files invalidate the cache when discovered at resolve or serving time. Session/view ownership, workspace realpath boundaries, descriptor identity, and citation/embedded checks remain per-request requirements.
