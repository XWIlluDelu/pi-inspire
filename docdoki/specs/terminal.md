...
- Last-output copy uses the latest completed shell-marker range in the normal buffer, including cell columns rather than guessed line offsets. It excludes the next prompt, joins soft wraps without corrupting Unicode or real spaces, and becomes unavailable when its markers are evicted or the view replaces/resets/reflows that buffer. This is a bounded view capability, not a durable command log; selected-text copy remains independent.
