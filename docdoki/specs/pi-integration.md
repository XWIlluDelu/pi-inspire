...
- Worker startup establishes a trusted projection tail (or trusted empty baseline), constructs the process, then immediately reconciles again before `rpc.start()` and requires identity/stat version, revision, tail, fingerprint, and committed bytes to be unchanged. Projection readers remain serialized behind the boundary. After start, the sole accepted delta is one exact missing `thinking_level_change` direct append whose level matches state and whose RPC entry equals the reconciled projection entry; only then does the writer baseline advance. This establishes state equivalence, not causal authorship: stock public RPC cannot distinguish an exactly matching entry from a prohibited concurrent writer in the interval after the second baseline. The one-writer rule remains authoritative, and additional, mixed, wrong-parent/value, path/session, filesystem-object, or rewrite divergence stops the worker.
