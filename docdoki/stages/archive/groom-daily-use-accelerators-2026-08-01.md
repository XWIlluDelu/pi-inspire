...
- Leading `/` completion combines Pi’s authoritative extension, prompt, and skill commands with inspire’s host-owned `/compact`. The unfiltered inventory is grouped by source; a typed query uses Pi TUI’s public command-name `fuzzyFilter` globally, so descriptions remain explanatory and cannot admit unrelated commands. Pi collisions preserve first wire ownership (extension before prompt/skill in Pi’s dispatch order), after which `/compact` is explicitly overridden by the host descriptor/parser that executes it. Selection edits the draft with one argument delimiter without sending, and candidate scrolling never returns a browser API value as a React effect cleanup.
...
- [x] Pure parser/ranking tests cover multiword project queries, caret-local replacement, command boundaries, source preservation, directory/basename ranking, Pi TUI command-name ordering, and rejection of description-only command matches.
...
- [x] The final focused and repository suites pass with 527 tests, together with TypeScript and the production build. A real Chromium regression covers `/`, `/f`, and no-match queries without a root crash, console error, or horizontal overflow.
