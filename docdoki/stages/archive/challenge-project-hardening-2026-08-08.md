...
- Complete: Dynamic mode gives fast Thinking blocks 1.8 seconds of minimum Expanded residency, fast Tool cards 1.6 seconds Expanded, and completed Tool batches 800 ms visibly Collapsed between the 180 ms transitions; lifecycle triggers, historical density, and reduced-motion behavior remain unchanged.
...
- Final isolated evaluator after the Dynamic dwell adjustment: 21 accepted samples, one event-loop-contaminated attempt discarded, exact `/api/resources/list` count of one per sample, `no-performance-change`, zero activated suspects. Changes p95 179.90 ms, History 115.70 ms, Files React 1.00 ms, projection 16.41 ms, Git status 12.42 ms.
...
The implementation and release verification are complete. Publish version `0.1.0` from the exact verified tarball built from its release commit; its Git tag, npm registry version, GitHub Release attachment, and checksum witness must all identify that same release.
