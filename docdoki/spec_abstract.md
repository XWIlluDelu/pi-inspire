...
| Resource preview | [[resource-preview]] | Explicit files and artifacts from Pi conversation context open in a session-bound, defensive preview surface rather than an unrestricted browser file client. |
| Visual language | [[visual-language]] | An original scientific-workbench character: reference grammar without reference identity, one accent per theme, IBM Plex as the single type voice. |
...
The front end uses one coherent light-and-dark component system at medium information density. IBM Plex Sans SC owns the interface and reading flow, IBM Plex Serif appears only in the wordmark, and IBM Plex Mono owns code and machine-oriented data. Neutral paper surfaces, one accent per theme (teal in light, industrial chartreuse in dark), fine boundaries, soft radii, and performance-safe state motion keep content dominant.
