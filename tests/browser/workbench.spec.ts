...
test("files workbench spans workspace navigation, detail modes, and narrow return", async ({
  page,
}) => {
  ...
  await expect(
    pane.getByRole("searchbox", { name: "Search workspace files" }),
  ).toBeVisible();
  await expect(
    pane.getByRole("button", { name: "All", exact: true }),
  ).toHaveAttribute("aria-pressed", "true");

  const tree = pane.getByRole("region", { name: "All files" });
  await tree.getByRole("button", { name: "src", exact: true }).click();
  await expect(
    tree.getByRole("button", { name: "App.tsx", exact: true }),
  ).toBeVisible();
  await tree.getByRole("button", { name: "README.md", exact: true }).click();
  await expect(
    pane.getByText("README.md", { exact: true }).first(),
  ).toBeVisible();
  await expect(pane.getByRole("tab", { name: "Preview" })).toHaveAttribute(
    "aria-selected",
    "true",
  );
  await pane.getByRole("tab", { name: "Source" }).click();
  await expect(pane.locator(".code-block__lines")).toBeVisible();
  await pane.getByRole("tab", { name: "Info" }).click();
  await expect(pane.getByText("Path", { exact: true })).toBeVisible();
  await expect(
    pane.getByText("README.md", { exact: true }).last(),
  ).toBeVisible();

  await page.setViewportSize({ width: 390, height: 844 });
  const drawer = page.getByRole("dialog", { name: "Files and resources" });
  await expect(drawer).toBeVisible();
  await expect(
    drawer.getByRole("button", { name: "Back to list" }),
  ).toBeVisible();
  await expect(drawer.getByRole("tab", { name: "Info" })).toHaveAttribute(
    "aria-selected",
    "true",
  );
  await drawer.getByRole("button", { name: "Back to list" }).click();
  await expect(
    drawer.getByRole("searchbox", { name: "Search workspace files" }),
  ).toBeVisible();
  await expect(
    drawer.locator('[data-workspace-path="README.md"]'),
  ).toHaveAttribute("aria-current", "true");

  await drawer.getByRole("button", { name: "Close resources panel" }).click();
  ...
  const selectedFile = navigation.locator('[data-workspace-path="README.md"]');
  ...
  await expect(
    page
      .getByRole("dialog", { name: "Files and resources" })
