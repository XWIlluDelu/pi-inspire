      ...
      providers: {
        "offline-compat": {
          ...
          models: [
            {
              id: "offline-model",
              name: "Offline compatibility model",
              reasoning: false,
              input: ["text"],
...
describe("installed Pi compatibility boundary", () => {
  it("keeps existing-session preview byte-preserving and checks RPC state/cursor/tree/model/command/stats", async () => {
    ...
    const preview = await loadSessionPreview({
      ...
      firstMessage: "first compatibility question",
    ...
    try {
      ...
      expect(
        all.entries.filter(
          (entry) =>
            ...
            ["u1", "a1", "u2", "a2", "u3", "a3"].includes(entry.id),
        ),
      ).toHaveLength(entries.length);
      expect(all.leafId).toBeTruthy();
      const bounded = await rpc.request<{
        entries: unknown[];
        leafId: string | null;
      ...
      const commands = await rpc.request<{
        commands: Array<{ name?: string; invocationName?: string }>;
      }>({ type: "get_commands" });
      expect(
        commands.commands.some(
          (command) =>
            command.name === "compat-probe" ||
            command.invocationName === "compat-probe",
        ),
      ).toBe(true);
      const stats = await rpc.request<Record<string, unknown>>({
  ...
  it("runs model selection, command UI, offline compaction, replacement, switch, and fork", async () => {
    ...
    rpc.on("event", (event) => {
      ...
      if (record.method === "select")
        void rpc.sendExtensionUiResponse({ id: record.id, value: "beta" });
      if (record.method === "confirm")
        void rpc.sendExtensionUiResponse({ id: record.id, confirmed: true });
      if (record.method === "input")
        void rpc.sendExtensionUiResponse({ id: record.id, value: "typed" });
      if (record.method === "editor")
        void rpc.sendExtensionUiResponse({ id: record.id, value: "edited" });
      if (record.method === "setStatus" && record.statusKey === "compat-result")
        result = JSON.parse(String(record.statusText));
    });
    await rpc.start();
    try {
      const models = await rpc.request<{
      ...
      const model = await rpc.request<{ provider: string; id: string }>({
        type: "set_model",
      ...
      expect(new Set(uiEvents.map((event) => event.method))).toEqual(
        new Set([
          "select",
          "confirm",
          "input",
      ...
      expect(appendedEvents[0]?.entry).toMatchObject({
        type: "custom",
        customType: "compat-provenance",
        data: { count: 2 },
      });
      const afterAppend = await rpc.request<{
        entries: Array<Record<string, unknown>>;
      }>({ type: "get_entries" });
      expect(
        afterAppend.entries.some(
          (entry) =>
            entry.type === "custom" && entry.customType === "compat-provenance",
        ),
      ).toBe(true);

      const compacted = await rpc.request<Record<string, unknown>>(
        { type: "compact" },
        60_000,
      );
      ...
      const afterCompact = await rpc.request<{
        entries: Array<Record<string, unknown>>;
      }>({ type: "get_entries" });
      expect(
        afterCompact.entries.some((entry) => entry.type === "compaction"),
      ).toBe(true);

      const replacement = await rpc.request<{ cancelled: boolean }>({
        type: "new_session",
        parentSession: sessionFile,
      });
      expect(replacement.cancelled).toBe(false);
      const replacementState = await rpc.request<{
        sessionId: string;
        sessionFile: string;
      }>({ type: "get_state" });
      expect(replacementState.sessionId).not.toBe(SOURCE_ID);
      expect(
        resolve(replacementState.sessionFile).startsWith(resolve(sessionDir)),
      ).toBe(true);
      ...
      ).toMatchObject({ sessionId: SOURCE_ID });
      ...
      expect(forked).toEqual({
        text: "second compatibility question",
        cancelled: false,
      });
      const forkState = await rpc.request<{
        sessionId: string;
        sessionFile: string;
      }>({ type: "get_state" });
