...
describe("branch bridge protocol framing", () => {
  ...
  it("independently enforces decoded bytes and encoded characters", () => {
    ...
    expect(() => encodeBranchBridgeJson("1234567890", 12)).toThrow(/encoded character/);
    const encoded = Buffer.from(JSON.stringify("1234567890"), "utf8").toString("base64url");
  ...
  it("rejects non-canonical and malformed UTF-8 framing", () => {
    ...
    expect(() => decodeBranchBridgeJson(Buffer.from([0xff]).toString("base64url"), 32)).toThrow();
  });
});
