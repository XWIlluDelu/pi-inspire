...
describe("project index authority", () => {
  ...
  it("fails closed when git exists but the index command fails", async () => {
    ...
    await writeFile(join(root, "app.js"), "code\n");
    await writeFile(join(root, ".git", "index"), "invalid\n");

    await expect(searchProjectFiles(root, "", 100)).rejects.toThrow();
  });

  it.runIf(process.platform !== "win32")(
    "does not widen to the filesystem after an incompatible Git probe",
    async () => {
      const root = await scratch();
      await writeFile(join(root, "app.js"), "code\n");
  ...
  it.runIf(process.platform !== "win32")(
    "rejects byte-distinct git paths that cannot be represented as UTF-8",
    async () => {
      const root = await scratch();
      const bin = join(root, "bin");
      await mkdir(bin);
  ...
  it.runIf(process.platform !== "win32")(
    "uses real git output without shell path quoting",
    async () => {
      const root = await scratch();
      await execFileAsync("git", ["-C", root, "init", "-q"]);
