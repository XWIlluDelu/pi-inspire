...
function setup(navigateTree: (target: string, options: unknown) => Promise<{ cancelled: boolean }> = vi.fn(async () => ({ cancelled: false }))) {
  ...
  let handler!: (argument: string, context: ExtensionCommandContext) => Promise<void>;
  const api = {
...
function argument(nonce = "nonce_abcdefghijklmnopqrstuvwxyz123456") {
  return Buffer.from(JSON.stringify({ v: 1, nonce, workerId, sessionId, operation: "navigate", targetId: "target" })).toString("base64url");
}

function decoded(statuses: Array<[string, string | undefined]>) {
  return JSON.parse(Buffer.from(statuses[0]![1]!, "base64url").toString("utf8")) as Record<string, unknown>;
}
...
describe("inspire branch extension", () => {
  it("uses public unsummarized navigation and emits one bounded final nonce result", async () => {
    ...
    expect(fixture.context.navigateTree).toHaveBeenCalledWith("target", { summarize: false });
  ...
  it.each(["%%%", "a".repeat(4_100)])("catches malformed input and still emits exactly one small error result", async (input) => {
    const fixture = setup();
    await expect(fixture.handler(input, fixture.context)).resolves.toBeUndefined();
    expect(fixture.statuses).toHaveLength(1);
    expect(decoded(fixture.statuses)).toMatchObject({ ok: false, cancelled: false, error: expect.any(String) });
  ...
  it("catches navigation errors and reports them instead of relying on prompt success", async () => {
    const fixture = setup(vi.fn(async () => { throw new Error("tree hook failed"); }));
    await expect(fixture.handler(argument(), fixture.context)).resolves.toBeUndefined();
    expect(fixture.statuses).toHaveLength(1);
    expect(decoded(fixture.statuses)).toMatchObject({ ok: false, error: "tree hook failed", beforeLeaf: "old-leaf" });
  });
});
