...
const entry = (id: string, parentId: string | null, role: "user" | "assistant", content: string, timestamp: number) => ({
...
class BranchRpc extends EventEmitter {
  ...
  leafId = "a2";
  stops = 0;
  resultMode: "ok" | "wrong-nonce" | "wrong-worker" | "malformed" | "duplicate" | "cancel" | "missing" | "persist" = "ok";
  ...
  private waitForDialog(id: string, method: "confirm" | "input"): Promise<void> {
    this.emit("event", { type: "extension_ui_request", id, method, title: `branch ${method}` });
    return new Promise<void>((resolveDialog) => this.dialogResolvers.set(id, resolveDialog));
  }
  ...
  private async emitBridge(message: string): Promise<void> {
    ...
    const result = {
      v: 1,
      nonce: this.resultMode === "wrong-nonce" ? "nonce_wrong_1234567890" : request.nonce,
      workerId: this.resultMode === "wrong-worker" ? "worker_wrong_abcdefghijklmnopqrstuvwxyz" : this.workerId,
    ...
    if (this.resultMode === "persist") {
      await appendFile(this.path, `${JSON.stringify(entry("unexpected", this.leafId, "assistant", "unexpected write", 60))}\n`);
    }
    ...
    const event = {
      ...
      statusText: this.resultMode === "malformed" ? "not+base64" : Buffer.from(JSON.stringify(result)).toString("base64url"),
    };
    this.emit("event", event);
    if (this.resultMode === "duplicate") this.emit("event", { ...event, id: "duplicate" });
  }
  ...
  async request<T>(command: Record<string, unknown>): Promise<T> {
    ...
    if (command.type === "get_state") {
      value = { sessionId: this.sessionId, sessionFile: this.path, model: null, thinkingLevel: "off", isStreaming: false, isCompacting: false };
    } else if (command.type === "get_available_models") value = { models: [] };
    else if (command.type === "get_commands") value = { commands: [{ name: this.bridgeCommand, source: "extension" }, { name: "visible", source: "extension" }] };
    else if (command.type === "get_entries") value = { entries: [], leafId: this.leafId };
      ...
      const destination = [
        { type: "session", version: 3, id: this.forkSessionId, timestamp: "2026-08-01T00:00:00.000Z", cwd: this.options.cwd },
        entry("u1", null, "user", "root", 1),
        entry("a1", "u1", "assistant", "first answer", 2),
        entry("alt", "u1", "assistant", "fork sibling", 5),
      ];
      await writeFile(this.forkPath, `${destination.map((line) => JSON.stringify(line)).join("\n")}\n`);
      ...
      if (text.startsWith(`/${this.bridgeCommand} `)) await this.emitBridge(text);
      else {
...
async function setup(
  ...
  const lines = [
    { type: "session", version: 3, id: SESSION_ID, timestamp: "2026-08-01T00:00:00.000Z", cwd: directory },
    entry("u1", null, "user", "root", 1),
    entry("a1", "u1", "assistant", "first answer", 2),
  ...
  const record: SessionRecord = {
    id: SESSION_ID, cwd: directory, path, created: new Date(), modified: new Date(), messageCount: 4,
    firstMessage: "root", searchText: "root",
  };
  const records = new Map([[record.id, record]]);
  const catalog: SessionCatalogLike = {
    refresh: async () => [...records.values()], get: async (id) => records.get(id),
    list: async () => ({ sessions: [], total: 0, offset: 0, limit: 40 }), listByIds: async () => [], listByCwds: async () => [], invalidate() {},
  };
  ...
  }, undefined, branchBridgeTimeoutMs, openForkProjection);
  await runtime.openSession(SESSION_ID);
  await vi.waitFor(() => expect(worker).toBeDefined());
  await vi.waitFor(async () => expect((await runtime.snapshot()).active?.commands).toBeDefined());
...
describe("stock RPC branch bridge", () => {
  it("keeps normal discovered extensions enabled in production argv while adding the explicit hidden bridge", async () => {
    ...
    try {
      ...
      expect(worker.options.args).toContain(resolve("server/extensions/inspire-branch-bridge.ts"));
      ...
      expect(before.active?.commands).toEqual([{ name: "visible", source: "extension" }]);
      const tree = await runtime.branchTree(SESSION_ID);
      const response = await runtime.navigateBranch({ sessionId: SESSION_ID, revision: tree.revision, targetId: "a1", mode: "switch" });
      expect(response.snapshot.active?.effectiveLeafId).toBe("a1");
      expect(response.snapshot.active?.navigationLeased).toBe(true);
      expect(JSON.stringify(response.snapshot.active?.messages)).toContain("first answer");
      expect(JSON.stringify(response.snapshot.active?.messages)).not.toContain("second question");
      expect(worker.commands.find((command) => command.type === "get_entries")).toMatchObject({ since: "a2" });
      await expect(runtime.navigateBranch({ sessionId: SESSION_ID, revision: tree.revision, targetId: "a2", mode: "switch" })).rejects.toThrow(/stale/);

      await runtime.prompt({ sessionId: SESSION_ID, message: "continued branch" });
      ...
      expect(JSON.stringify(committed.active?.messages)).toContain("continued branch");
    } finally {
      await runtime.close();
    }
  });
  ...
  it("answers a session_before_tree dialog on the independent response lane without deadlock", async () => {
    ...
    try {
      const tree = await runtime.branchTree(SESSION_ID);
      const navigating = runtime.navigateBranch({ sessionId: SESSION_ID, revision: tree.revision, targetId: "a1", mode: "switch" });
      await vi.waitFor(async () => expect((await runtime.snapshot()).pendingExtensionUiRequests).toMatchObject([{ id: "tree-hook", sessionId: SESSION_ID }]));
      await runtime.extensionUiResponse({ sessionId: SESSION_ID, id: "tree-hook", value: true });
      await expect(navigating).resolves.toMatchObject({ snapshot: { active: { effectiveLeafId: "a1" } } });
      expect(worker.extensionResponses.filter((response) => response.id === "tree-hook")).toHaveLength(1);
      await expect(runtime.extensionUiResponse({ sessionId: SESSION_ID, id: "tree-hook", value: true })).rejects.toMatchObject({ status: 409 });
    } finally {
      await runtime.close();
    }
  });
  ...
  it("emits a session_before_fork dialog under the source id and rebinds only unresolved requests", async () => {
    ...
    try {
      ...
      await vi.waitFor(() => expect(events.find((event) => event.id === "fork-hook")).toMatchObject({ sessionId: SESSION_ID }));
      await runtime.extensionUiResponse({ sessionId: SESSION_ID, id: "fork-hook", value: "continue" });
      ...
      expect(worker.extensionResponses.filter((response) => response.id === "fork-hook")).toHaveLength(1);
    } finally {
      await runtime.close();
    }
  });
  ...
  it("returns original edit text, navigates to the parent, and refuses the irreducible root case", async () => {
    ...
    try {
      const tree = await runtime.branchTree(SESSION_ID);
      const edited = await runtime.navigateBranch({ sessionId: SESSION_ID, revision: tree.revision, targetId: "u2", mode: "edit" });
      expect(edited.editorText).toBe("second question");
      expect(edited.snapshot.active?.effectiveLeafId).toBe("a1");
      await expect(runtime.navigateBranch({ sessionId: SESSION_ID, revision: tree.revision, targetId: "u1", mode: "edit" })).rejects.toMatchObject({ status: 409 });
    } finally {
      await runtime.close();
    }
  });

  it.each(["wrong-nonce", "wrong-worker", "malformed", "duplicate"] as const)("fails closed for a %s result and stops the ambiguous worker", async (mode) => {
    const { runtime, worker } = await setup();
    worker.resultMode = mode;
    const tree = await runtime.branchTree(SESSION_ID);
    await expect(runtime.navigateBranch({ sessionId: SESSION_ID, revision: tree.revision, targetId: "a1", mode: "switch" })).rejects.toMatchObject({ status: 504 });
  ...
  it("buffers fork replacement events, atomically rebinds identity, and keeps the bridge usable", async () => {
    ...
    try {
      ...
      const forked = await runtime.forkBranch({ sessionId: SESSION_ID, revision: sourceTree.revision, targetId: "u2" });
      ...
      expect(events.find((event) => event.type === "session_start")).toMatchObject({ sessionId: worker.forkSessionId });
      expect(events.some((event) => event.sessionId === SESSION_ID && event.type === "session_start")).toBe(false);
      const slots = (runtime as unknown as { slots: Map<string, { projection: unknown }> }).slots;
      await vi.waitFor(() => expect(slots.has(SESSION_ID)).toBe(false));
      expect([...slots.values()].filter((slot) => slot.projection !== null)).toHaveLength(1);
  ...
  it("releases a failed fork reservation so destination open can recover with one fresh worker", async () => {
    ...
    try {
      ...
      await expect(openingDestination).resolves.toMatchObject({ active: { sessionId: worker.forkSessionId } });
  ...
  it("treats cancellation as a truthful refusal without moving or stopping the worker", async () => {
    ...
    const tree = await runtime.branchTree(SESSION_ID);
    try {
      await expect(runtime.navigateBranch({ sessionId: SESSION_ID, revision: tree.revision, targetId: "a1", mode: "switch" })).rejects.toMatchObject({ status: 409 });
  ...
  it("drops a stale unsolicited internal status without projecting it or poisoning the next operation", async () => {
    ...
    runtime.on("event", (event) => forwarded.push(event as Record<string, unknown>));
    worker.emit("event", { type: "extension_ui_request", id: "stale", method: "setStatus", statusKey: worker.statusKey, statusText: "not+base64" });
    const tree = await runtime.branchTree(SESSION_ID);
    try {
      await runtime.navigateBranch({ sessionId: SESSION_ID, revision: tree.revision, targetId: "a1", mode: "switch" });
      expect(forwarded.some((event) => event.id === "stale")).toBe(false);
      expect(forwarded.some((event) => event.statusKey === worker.statusKey)).toBe(false);
      expect(worker.stops).toBe(0);
    } finally {
      await runtime.close();
    }
  ...
  it("clears fork rebinding state after invalid final identity so a fresh source worker can recover", async () => {
    ...
    try {
      const tree = await runtime.branchTree(SESSION_ID);
      await expect(runtime.forkBranch({ sessionId: SESSION_ID, revision: tree.revision, targetId: "u2" })).rejects.toMatchObject({ status: 409 });
      expect(worker.stops).toBe(1);
      await expect(runtime.prompt({ sessionId: SESSION_ID, message: "recover source" })).resolves.toBeUndefined();
      expect(JSON.stringify((await runtime.snapshot()).active?.messages)).toContain("recover source");
    } finally {
      await runtime.close();
    }
  });
  ...
  it("stops buffering and fails closed when the fork event cap is exceeded", async () => {
    ...
    try {
      const tree = await runtime.branchTree(SESSION_ID);
      await expect(runtime.forkBranch({ sessionId: SESSION_ID, revision: tree.revision, targetId: "u2" })).rejects.toMatchObject({ status: 504 });
      expect(worker.stops).toBe(1);
  ...
  it("does not let a slower fork steal a newer host selection intent", async () => {
    ...
    let release!: () => void;
    let started!: () => void;
    worker.forkGate = new Promise<void>((resolve) => { release = resolve; });
  ...
  it("rejects direct public invocation of the randomized internal command", async () => {
    ...
    try {
      for (const suffix of [" forged", "\tforged", "\nforged", ""]) {
        await expect(runtime.prompt({ sessionId: SESSION_ID, message: `/${worker.bridgeCommand}${suffix}` })).rejects.toMatchObject({ status: 403 });
      }
      expect(worker.commands.filter((command) => command.type === "prompt")).toHaveLength(0);
    } finally {
      await runtime.close();
    }
  });
  ...
  it("conflicts on external divergence while a navigation lease owns the worker", async () => {
    ...
    try {
      const tree = await runtime.branchTree(SESSION_ID);
      await runtime.navigateBranch({ sessionId: SESSION_ID, revision: tree.revision, targetId: "a1", mode: "switch" });
      await appendFile(path, `${JSON.stringify(entry("external", "a2", "assistant", "terminal divergence", 60))}\n`);
      await expect(runtime.branchTree(SESSION_ID)).rejects.toThrow(/changed on disk/);
      expect((await runtime.snapshot()).runState).toBe("conflict");
    } finally {
      await runtime.close();
    }
