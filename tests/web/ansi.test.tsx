...
describe("stripTerminalSequences", () => {
  it("strips SGR truecolor and reset sequences", () => {
    expect(
      stripTerminalSequences(
        `${ESC}[38;2;90;128;128mteal thought${ESC}[0m done`,
      ),
    ).toBe("teal thought done");
  });

  it("strips compound and cursor CSI sequences", () => {
    expect(
      stripTerminalSequences(
        `${ESC}[1;31mbold red${ESC}[m${ESC}[2K${ESC}[1Gplain`,
      ),
    ).toBe("bold redplain");
  });
  ...
  it("strips OSC hyperlinks terminated by ST", () => {
    expect(
      stripTerminalSequences(
        `${ESC}]8;;https://example.com${ESC}\\link${ESC}]8;;${ESC}\\ end`,
      ),
    ).toBe("link end");
  });

  it("strips OSC sequences terminated by BEL", () => {
    expect(stripTerminalSequences(`${ESC}]0;window title\u0007visible`)).toBe(
      "visible",
    );
  });
  ...
  it("leaves ordinary text and Markdown untouched", () => {
    const markdown =
      "**bold** and [a link](https://pi.dev) plus `code` — 38;2;90 stays when not a sequence";
...
describe("thinking display boundary", () => {
  const message = {
    ...
    content: [
      {
        type: "thinking",
        thinking: `${ESC}[38;2;90;128;128mcolored thought${ESC}[0m\nsecond line`,
      },
    ],
    timestamp: 1,
  };
