...
import { bootstrapPayload, FakeWebSocket, installFakeWebSocket, installFetch, jsonBody, sessionSummary } from "./helpers";

let olderAttempts = 0;
let failPreservedRefresh = false;
let failVisibleHydration = false;
let releaseFirstOlder!: () => void;
const firstOlderGate = new Promise<void>((resolve) => { releaseFirstOlder = resolve; });
  ...
  installFetch(async (url, init) => {
    ...
    if (url.startsWith("/api/sessions?")) {
      ...
      if (failPreservedRefresh && offset === 0 && limit === 3) {
        return { status: 503, body: { error: "Could not preserve loaded sessions" } };
      }
      if (offset === 0) return { body: {
        sessions: [sessionSummary({ id: "page-1", title: "Newest" })], total: 3, offset, limit: 40,
      } };
      if (offset === 1) {
        olderAttempts += 1;
        if (olderAttempts === 1) {
        ...
        return { body: {
          sessions: [sessionSummary({ id: "page-2", title: "Middle" })], total: 3, offset, limit: 40,
        } };
      }
      return { body: {
        sessions: [sessionSummary({ id: "page-3", title: "Oldest" })], total: 3, offset, limit: 40,
      } };
    }
    return undefined;
  });
...
describe("session pagination control", () => {
  it("is keyboard-complete, reports actionable states, and omits redundant completion chrome", async () => {
    ...
    const load = within(nav).getByRole("button", { name: "Load older sessions" });
    load.focus();
    await user.keyboard("{Enter}");
    const loading = within(nav).getByRole("button", { name: "Loading older sessions…" });
    expect(loading).toBeDisabled();
    expect(loading).toHaveAttribute("aria-busy", "true");
    releaseFirstOlder();
    const retry = await within(nav).findByRole("button", { name: "Retry loading older sessions" });
    expect(store.getState().sessions.map((session) => session.id)).toEqual(["page-1"]);
