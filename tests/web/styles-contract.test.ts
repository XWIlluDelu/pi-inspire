...
describe("design token contract", () => {
  ...
  it("keeps navigation run outcomes on distinct traffic-light tokens", async () => {
    const css = await readFile(stylesheet, "utf8");
    expect(css).toMatch(/\.nav__row-dot--running\s*{[^}]*background:\s*var\(--warning\)/s);
    expect(css).toMatch(/\.nav__row-dot--completed\s*{[^}]*background:\s*var\(--success\)/s);
    expect(css).toMatch(/\.nav__row-dot--failed\s*{[^}]*background:\s*var\(--error\)/s);
