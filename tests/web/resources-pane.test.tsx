...
describe("Files pane", () => {
  ...
  beforeEach(async () => {
    ...
    vi.stubGlobal("fetch", vi.fn(async (input: string | URL | Request, init?: RequestInit) => {
      ...
      const gitPath = (id: string, display: string, workspacePath?: string) => ({
        id, display, utf8Path: display, ...(workspacePath ? { workspacePath } : {}),
      });
      if (url.startsWith("/api/git/status")) {
        if (gitStatusFails) return Response.json({ error: "Git timed out" }, { status: 503 });
        return Response.json({
        kind: "repository",
        head: { kind: "branch", name: "feature/git", oid: "0123456789abcdef" },
        files: [
          { path: gitPath("notes", "notes.md", "notes.md"), unstaged: { kind: "modified" }, untracked: false },
          { path: gitPath("both", "both.ts", "both.ts"), staged: { kind: "modified" }, unstaged: { kind: "modified" }, untracked: false },
          { path: gitPath("rename", "renamed.ts", "renamed.ts"), staged: { kind: "renamed", originalPath: gitPath("old", "old.ts", "old.ts") }, untracked: false },
          { path: gitPath("copy", "copied.ts", "copied.ts"), staged: { kind: "copied", originalPath: gitPath("source", "source.ts", "source.ts") }, untracked: false },
          { path: gitPath("conflict", "conflict.ts", "conflict.ts"), conflict: { code: "UU" }, untracked: false },
          { path: gitPath("binary", "image.bin", "image.bin"), unstaged: { kind: "modified" }, untracked: false },
          { path: gitPath("module", "vendor/module", "vendor/module"), staged: { kind: "modified" }, untracked: false, submodule: { commitChanged: true, trackedModified: false, untracked: false } },
          { path: gitPath("deleted", "gone.ts", "gone.ts"), unstaged: { kind: "deleted" }, untracked: false },
          { path: gitPath("outside", "outside.ts"), unstaged: { kind: "modified" }, untracked: false },
          { path: { id: "bytes", display: "bad\\xff" }, unstaged: { kind: "modified" }, untracked: false },
          { path: gitPath("new", "new.ts", "new.ts"), untracked: true },
        ],
        total: 1_005,
        truncated: true,
        groups: {
          conflicted: ["conflict"], staged: ["both", "rename", "copy", "module"],
          unstaged: ["notes", "both", "binary", "deleted", "outside", "bytes"], untracked: ["new"],
        },
      });
      }
      if (url.startsWith("/api/git/diff")) {
        const body = JSON.parse(String(init?.body ?? "{}")) as { pathId: string; side: "staged" | "unstaged" };
        const paths: Record<string, GitPathIdentity> = {
          notes: gitPath("notes", "notes.md", "notes.md"), both: gitPath("both", "both.ts", "both.ts"), rename: gitPath("rename", "renamed.ts", "renamed.ts"), copy: gitPath("copy", "copied.ts", "copied.ts"), conflict: gitPath("conflict", "conflict.ts", "conflict.ts"),
          binary: gitPath("binary", "image.bin", "image.bin"), module: gitPath("module", "vendor/module", "vendor/module"),
          deleted: gitPath("deleted", "gone.ts", "gone.ts"), outside: gitPath("outside", "outside.ts"),
          bytes: { id: "bytes", display: "bad\\xff" }, new: gitPath("new", "new.ts", "new.ts"),
        };
        const base = { path: paths[body.pathId], side: body.side };
        if (body.pathId === "binary") return Response.json({ ...base, kind: "binary" });
        if (body.pathId === "module") return Response.json({ ...base, kind: "submodule", state: { commitChanged: true, trackedModified: false, untracked: false } });
        if (body.pathId === "conflict") return Response.json({ ...base, kind: "conflict", code: "UU" });
        if (body.pathId === "bytes") return Response.json({ ...base, kind: "unsupported", reason: "path-encoding" });
        if (body.pathId === "new") return Response.json({ ...base, kind: "unsupported", reason: "untracked-content" });
        return Response.json({ ...base, kind: "text", truncated: body.pathId === "both", encodingLossy: false, lines: [
          { kind: "hunk", text: "@@ -1 +1 @@", oldLine: null, newLine: null },
      ...
      if (url.startsWith("/api/resources/probe")) {
        ...
        return Response.json({
          sessionId: "s1",
          viewId: "view-s1",
          results: body.references.map((reference) => reference === missingProbeReference
  ...
  it("renders grouped canonical changes, side selection, numbered diffs, and explicit result states", async () => {
    ...
    expect(screen.getByRole("button", { name: "conflict.ts, conflict UU" })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "renamed.ts, staged renamed, renamed from old.ts" })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "copied.ts, staged copied, copied from source.ts" })).toBeInTheDocument();
    ...
    expect(await screen.findByLabelText("Diff for both.ts")).toBeInTheDocument();
    ...
    expect(await screen.findByText("Unsupported path encoding")).toBeInTheDocument();
    fireEvent.click(screen.getByRole("button", { name: /new\.ts/ }));
    expect(await screen.findByText("Untracked diff unavailable")).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Open File" })).toBeInTheDocument();
    fireEvent.click(screen.getByRole("button", { name: /outside\.ts/ }));
    expect(await screen.findByText(/outside the session workspace/)).toBeInTheDocument();
    fireEvent.click(screen.getByRole("button", { name: /gone\.ts/ }));
    expect(await screen.findByText(/working-tree file is deleted/)).toBeInTheDocument();
  });
  ...
  it("marks a missing reference from preflight before the row is selected", async () => {
    ...
    const missing = await screen.findByRole("button", { name: /demo\.html.*missing/i });
    expect(missing).toHaveClass("res__row--unavailable");
    expect(missing).toHaveAttribute("title", expect.stringContaining("not found"));
