...
import { compactAge, groupSessionsByCwd, splitNavSections, type NavCuration } from "../../src/components/Nav";
...
describe("splitNavSections", () => {
  it("collects pinned sessions across projects, newest activity first", () => {
    const a = sessionSummary({ id: "a", cwd: "/home/u/alpha", modified: "2026-07-20T10:00:00Z" });
    const b = sessionSummary({ id: "b", cwd: "/home/u/beta", modified: "2026-07-22T10:00:00Z" });
    const c = sessionSummary({ id: "c", cwd: "/home/u/alpha", modified: "2026-07-21T10:00:00Z" });
    const { pinned, groups } = splitNavSections([a, c, b], curation({ pinnedSessionIds: ["a", "b"] }));
  ...
  it("returns an empty Pinned section when nothing is pinned and empty groups when all are pinned", () => {
    ...
    expect(splitNavSections([only], curation({ pinnedSessionIds: ["y"] })).groups).toEqual([]);
  });
  ...
  it("keeps pinned matches in the Pinned section for a search-filtered subset", () => {
    ...
    const pinnedMatch = sessionSummary({ id: "pm", title: "fix parser", cwd: "/a" });
  ...
  it("lifts pinned folders out of the ordinary groups, keeping their own order", () => {
    const old = sessionSummary({ id: "old", cwd: "/home/u/archive", modified: "2026-07-01T10:00:00Z" });
    const fresh = sessionSummary({ id: "fresh", cwd: "/home/u/active", modified: "2026-07-25T10:00:00Z" });
  ...
  it("files a hidden session under Hidden even when its folder or the session itself is pinned", () => {
    ...
    const sections = splitNavSections(
      ...
      curation({ hiddenSessionIds: ["h"], pinnedSessionIds: ["h"], pinnedProjectCwds: ["/home/u/alpha"] }),
    );
    expect(sections.hidden.map((session) => session.id)).toEqual(["h"]);
    expect(sections.pinned).toEqual([]);
    expect(sections.pinnedGroups.flatMap((group) => group.sessions.map((s) => s.id))).toEqual(["k"]);
    expect(sections.groups).toEqual([]);
  });
});
