...
describe("PaneResizeHandle responsive bounds", () => {
  beforeEach(() => {
    ...
    vi.spyOn(HTMLElement.prototype, "getBoundingClientRect").mockImplementation(function (this: HTMLElement) {
      const width = this.classList.contains("ctx")
        ? Number.parseFloat(document.documentElement.style.getPropertyValue("--ctx-w")) || 500
        : 0;
      return {
        x: 0, y: 0, top: 0, right: width, bottom: 800, left: 0,
  ...
  it("temporarily clamps a saved pane width when the window narrows and restores it when expanded", async () => {
    ...
    const { PaneResizeHandle } = await import("../../src/components/PaneResizeHandle");
    render(
      <>
        <aside className="ctx" />
        <PaneResizeHandle
    ...
    expect(document.documentElement.style.getPropertyValue("--ctx-w")).toBe("700px");

    act(() => {
      setViewport(980);
      window.dispatchEvent(new Event("resize"));
    });
    expect(document.documentElement.style.getPropertyValue("--ctx-w")).toBe("340px");
