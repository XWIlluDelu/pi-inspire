...
import { activeSnapshot, bootstrapPayload, FakeWebSocket, installFakeWebSocket, installFetch, type RouteResponse } from "./helpers";

function branchTree(): BranchTreeResponse {
  return {
    sessionId: "s1", revision: 1, incarnation: "tree-1", durableLeafId: "a1", effectiveLeafId: null,
    activePath: ["u1", "a1"], truncated: false, health: { status: "ok" },
    nodes: [
      { id: "u1", parentId: null, depth: 0, type: "message", role: "user", label: "user", snippet: "user", timestamp: "2026-08-01", active: true, leaf: false, canSwitch: false, canEdit: false, canFork: true },
      { id: "a1", parentId: "u1", depth: 1, type: "message", role: "assistant", label: "assistant", snippet: "answer", timestamp: "2026-08-01", active: true, leaf: true, canSwitch: true, canEdit: false, canFork: false },
    ],
  };
}
...
async function setup(route: (url: string) => Promise<RouteResponse | undefined> | RouteResponse | undefined) {
  ...
  (store as unknown as { set(value: unknown): void }).set({ branchTree: branchTree() });
  return store;
}
...
function moveView(store: AppStore, viewId = "view-2"): void {
  const snapshot = activeSnapshot({
    transcriptPage: {
      sessionId: "s1", revision: 2, viewId, incarnation: "projection-1", appendFromRevision: 2,
      effectiveLeafId: "u1", messages: [], hasOlder: false, olderCursor: null,
    },
    effectiveLeafId: "u1",
  });
  (store as unknown as { applySnapshot(snapshot: unknown): void }).applySnapshot(snapshot);
}
...
describe("branch request view ownership", () => {
  ...
  it("ignores a delayed tree success after a same-session view transition", async () => {
    ...
    const store = await setup((url) => url.startsWith("/api/branches/tree") ? pending.promise : undefined);
  ...
  it("ignores a delayed navigation error after a same-session view transition", async () => {
    const pending = deferred<RouteResponse | undefined>();
    const store = await setup((url) => url.startsWith("/api/branches/navigate") ? pending.promise : undefined);
  ...
  it("ignores delayed fork success after reconnect invalidates ownership", async () => {
    ...
    fork.resolve({ body: { sessionId: "forked", snapshot: activeSnapshot({ sessionId: "forked" }), editorText: "old" } });
    await expect(forking).resolves.toBe(false);
    expect(store.getState().sessionId).toBe("s1");
    expect(store.getState().branchActionId).toBeNull();
  });

  it.each(["tree", "navigate", "fork"] as const)("suppresses a delayed branch %s 401 only after the auth transport is genuinely replaced", async (kind) => {
    const pending = deferred<RouteResponse | undefined>();
    const store = await setup((url) => url.startsWith(`/api/branches/${kind}`) ? pending.promise : undefined);
    const request = kind === "tree" ? store.loadBranchTree()
      : kind === "navigate" ? store.navigateBranch("a1", "switch")
      : store.forkBranch("u1");
    await store.init("replacement-token");
    pending.resolve({ status: 401, body: { error: "expired old transport" } });
    await request;
    expect(store.getState().needsToken).toBe(false);
  });

  it.each(["tree", "navigate", "fork"] as const)("honors a delayed branch %s 401 after a view transition on the current transport", async (kind) => {
    const pending = deferred<RouteResponse | undefined>();
    const store = await setup((url) => url.startsWith(`/api/branches/${kind}`) ? pending.promise : undefined);
    const request = kind === "tree" ? store.loadBranchTree()
      : kind === "navigate" ? store.navigateBranch("a1", "switch")
      : store.forkBranch("u1");
    moveView(store);
    pending.resolve({ status: 401, body: { error: "current transport unauthorized" } });
    await request;
    expect(store.getState()).toMatchObject({ needsToken: true, connection: "offline", error: null });
  });

  it.each(["tree", "navigate", "fork"] as const)("settles delayed branch %s ownership across failed and successful selection intents", async (kind) => {
    for (const selectionOutcome of ["failed", "successful"] as const) {
      ...
      const branchRequest = kind === "tree" ? store.loadBranchTree()
        : kind === "navigate" ? store.navigateBranch("a1", "switch")
        : store.forkBranch("u1");
      const selection = store.openSession("s2");
      expect(store.getState()).toMatchObject({ branchTreeLoading: false, branchActionId: null });
      expect(store.getState().branchTreeError).toMatch(/stale/);

      if (selectionOutcome === "failed") opening.resolve({ status: 500, body: { error: "selection failed" } });
      else opening.resolve({ body: activeSnapshot({ sessionId: "s2" }) });
      await selection;
      branch.resolve(selectionOutcome === "failed"
        ? { status: 500, body: { error: "obsolete branch failure" } }
        : kind === "tree"
          ? { body: { ...branchTree(), revision: 99 } }
          : kind === "navigate"
            ? { body: { snapshot: activeSnapshot(), editorText: "obsolete" } }
            : { body: { sessionId: "forked", snapshot: activeSnapshot({ sessionId: "forked" }), editorText: "obsolete" } });
      await branchRequest;
  ...
  it.each(["navigate", "fork"] as const)("routes an owned branch %s 401 through auth loss", async (kind) => {
    const store = await setup((url) => {
      if (url.startsWith(`/api/branches/${kind}`)) return { status: 401, body: { error: "unauthorized" } };
      return undefined;
    });
