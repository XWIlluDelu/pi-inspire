...
describe("virtualized transcript search navigation", () => {
  ...
  it("mounts and follows the virtual tail across the threshold and same-message tool growth", () => {
    const messages = Array.from({ length: 60 }, (_, index) => ({
      ...
      timestamp: index,
      ...(index === 59
        ? { __inspireLiveId: "virtual-tail", __inspireSettled: false }
        : {}),
    }));
    const props = {
      streaming: true,
    ...
    rerender(
      <Transcript sessionId="virtual-follow" messages={messages} {...props} />,
    );
    expect(virtual.scrollToIndex).toHaveBeenLastCalledWith(59, {
      align: "end",
    });
  ...
  it("navigates by transcript row index and disables pinned latest-follow", () => {
    ...
    expect(virtual.scrollToIndex).toHaveBeenLastCalledWith(45, {
      align: "center",
    });
    expect(
  ...
  it("keeps a prepended search match anchored when a near-bottom scroll precedes a live append", async () => {
    ...
    expect(virtual.scrollToIndex).toHaveBeenLastCalledWith(60, {
      align: "center",
    });
