...
const searchable = distFiles.filter((path) => /\.(?:html|js|css|map)$/u.test(path));
if (searchable.length === 0) throw new Error("Production bundle is absent; run npm run build first");
...
if (leaks.length > 0) throw new Error(`Benchmark instrumentation leaked into production:\n${leaks.join("\n")}`);
...
for (const content of ["navigationContent", "transcriptContent", "composerContent", "resourcesContent"]) {
  if (!appSource.includes(`) : ${content}`)) throw new Error(`Production direct-element fallback is missing for ${content}`);
}

console.log(JSON.stringify({
  productionFilesScanned: searchable.length,
  sourceMapsScanned: searchable.filter((path) => path.endsWith(".map")).length,
  forbiddenSymbols: FORBIDDEN_PRODUCTION_SYMBOLS,
  benchmarkSymbolsFound: 0,
  productionFallbacks: ["navigation", "transcript", "composer", "resources"],
