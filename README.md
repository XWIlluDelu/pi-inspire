...
| Web UI in Firefox and WebKit | CI tested | Browser engine tested on Linux | Browser engine tested on Linux |
...
The browser suite runs the same end-to-end workbench scenarios in Chromium, Firefox, and WebKit, rather than maintaining browser-specific implementations.
...
The optional Linux-only `ssh-reverse` connection module opens a loopback-only reverse SSH tunnel from the local host to a user-controlled HTTPS edge. It is separate from the cross-platform core Host lifecycle and may be started, inspected, stopped, or installed as a systemd user service independently:
