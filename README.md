...
- **Adaptive activity cards.** Thinking and tool activity render as distinguishable, inspectable cards with file links and status. The default Dynamic mode keeps current work Expanded, moves each completed tool to a full-card Compact state, and reduces a completed multi-card batch to a Collapsed strip at the next model call; fixed Expanded, Compact, Collapsed, and Hidden choices remain available independently.
