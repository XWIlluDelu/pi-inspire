...
function controlSafeDisplay(value: Buffer): string {
  ...
  if (decoded !== undefined) {
    return decoded.replace(/\\/g, "\\\\").replace(/\n/g, "\\n").replace(/\r/g, "\\r").replace(/\t/g, "\\t").replace(/[\0-\x08\x0b\x0c\x0e-\x1f\x7f]/g, (character) =>
      `\\x${character.charCodeAt(0).toString(16).padStart(2, "0")}`,
    );
  }
  let result = "";
  for (const byte of value) {
    result += byte >= 0x20 && byte <= 0x7e && byte !== 0x5c
...
function safeRepositoryPath(value: string): boolean {
  if (!value || value.includes("\0") || value.startsWith("/") || value.split("/").some((part) => part === "." || part === "..")) return false;
  return true;
}

function underPrefix(path: Buffer, prefix: Buffer): Buffer | undefined {
  if (prefix.length === 0) return path;
  if (path.length <= prefix.length || !path.subarray(0, prefix.length).equals(prefix)) return undefined;
...
function identity(raw: Buffer, workspacePrefix: Buffer): GitPathIdentity {
  ...
  if (relative) {
    ...
    if (workspacePath !== undefined && safeRepositoryPath(workspacePath)) result.workspacePath = workspacePath;
  }
  return result;
}

function splitFixed(record: Buffer, spaces: number): { fields: string[]; path: Buffer } {
  const fields: string[] = [];
  let start = 0;
  for (let index = 0; index < spaces; index += 1) {
    const end = record.indexOf(0x20, start);
    if (end < 0) throw new GitInspectionError("Git returned malformed status data");
  ...
  if (path.length === 0 || path.includes(0)) throw new GitInspectionError("Git returned malformed status data");
  return { fields, path };
}

function validateTrackedFields(fields: string[], type: string, modeIndexes: number[], hashIndexes: number[]): void {
...
function deltaKind(code: string): GitDeltaKind | undefined {
  ...
  if (!kind) throw new GitInspectionError("Git returned an unknown change status");
  return kind;
}

function facets(xy: string, originalPath?: GitPathIdentity): Pick<GitFileChange, "staged" | "unstaged"> {
  ...
  const facet = (kind: GitDeltaKind): GitDeltaFacet => ({
    kind,
    ...(originalPath && (kind === "renamed" || kind === "copied") ? { originalPath } : {}),
  });
  return {
...
function submoduleState(field: string): GitSubmoduleState | undefined {
  ...
  if (!/^S[.C][.M][.U]$/.test(field)) throw new GitInspectionError("Git returned malformed submodule status");
  return {
...
function validBranchRef(value: Buffer): boolean {
  ...
  if (value[0] === 0x2f || value.at(-1) === 0x2f || value.at(-1) === 0x2e) return false;
  const text = value.toString("latin1");
  if (text.includes("//") || text.includes("..") || text.includes("@{")) return false;
  if (value.some((byte) => byte < 0x20 || byte === 0x7f || " ~^:?*[\\".includes(String.fromCharCode(byte)))) return false;
  return text.split("/").every((component) =>
    component.length > 0 && !component.startsWith(".") && !component.endsWith(".lock"));
}

function validBranchAb(value: Buffer): boolean {
  if (value.length === 0 || value.length > MAX_BRANCH_AB_BYTES || value.some((byte) => byte > 0x7f)) return false;
...
export function parsePorcelainV2(output: Buffer, workspacePrefix = Buffer.alloc(0)): ParsedRepository {
  if (output.length === 0 || output[output.length - 1] !== 0) {
    throw new GitInspectionError("Git returned incomplete status data");
  }
  const records = output.subarray(0, -1).toString("latin1").split("\0").map((part) => Buffer.from(part, "latin1"));
  ...
  for (let index = 0; index < records.length; index += 1) {
    ...
    if (record.subarray(0, 13).toString("ascii") === "# branch.oid ") {
      if (branchOid !== undefined) throw new GitInspectionError("Git returned duplicate branch data");
    ...
    if (record.subarray(0, 14).toString("ascii") === "# branch.head ") {
      if (branchHead !== undefined) throw new GitInspectionError("Git returned duplicate branch data");
    ...
    if (record.subarray(0, 18).toString("ascii") === "# branch.upstream ") {
      if (branchUpstreamSeen) throw new GitInspectionError("Git returned duplicate branch data");
      branchUpstreamSeen = true;
      if (!validBranchRef(record.subarray(18))) throw new GitInspectionError("Git returned malformed branch data");
      continue;
    }
    if (record.subarray(0, 12).toString("ascii") === "# branch.ab ") {
      if (branchAbSeen) throw new GitInspectionError("Git returned duplicate branch data");
      branchAbSeen = true;
      if (!validBranchAb(record.subarray(12))) throw new GitInspectionError("Git returned malformed branch data");
      continue;
    }
    ...
    if (type === "1") {
      ...
      if (!original || original.length === 0) throw new GitInspectionError("Git returned an incomplete rename record");
      ...
      const renameFacets = [...xy!].filter((value) => value === "R" || value === "C");
    ...
    total += 1;
    if (seenIds.has(change.path.id)) throw new GitInspectionError("Git returned duplicate path identities");
  ...
  if (branchOid === undefined || branchHead === undefined) throw new GitInspectionError("Git omitted branch identity");
  if (branchUpstreamSeen !== branchAbSeen) throw new GitInspectionError("Git returned incomplete branch tracking data");
  if (branchOid !== "(initial)" && !/^(?:[0-9a-f]{40}|[0-9a-f]{64})$/.test(branchOid)) {
    throw new GitInspectionError("Git returned malformed branch data");
  }
  let head: GitHead;
  if (branchOid === "(initial)") {
    if (branchHead === "(detached)") throw new GitInspectionError("Git returned malformed unborn head data");
  ...
  const response: ParsedRepository["response"] = {
    ...
    groups: {
      conflicted: files.filter((file) => file.conflict).map((file) => file.path.id),
      staged: files.filter((file) => file.staged).map((file) => file.path.id),
      unstaged: files.filter((file) => file.unstaged).map((file) => file.path.id),
      untracked: files.filter((file) => file.untracked).map((file) => file.path.id),
    },
  };
  return { response, rawById };
}

function decodePatchLines(output: Buffer, outputTruncated: boolean): {
  lines: string[];
  ...
  if (outputTruncated) {
    ...
    usable = lastNewline < 0 ? Buffer.alloc(0) : usable.subarray(0, lastNewline + 1);
  }
  ...
  const lines = lineBuffers.slice(0, contentLimit).map((line) => line.toString("utf8"));
  if (truncated) {
    lines.push(cardinalityTruncated
...
export function parseUnifiedDiff(output: Buffer, outputTruncated = false): Pick<Extract<GitDiffResponse, { kind: "text" }>, "lines" | "encodingLossy" | "truncated"> {
  ...
  return { lines, encodingLossy: decoded.encodingLossy, truncated: decoded.truncated };
}

async function hasGitMarker(cwd: string): Promise<boolean> {
  let current = resolve(cwd);
  try {
    if (!(await stat(current)).isDirectory()) throw new GitInspectionError("The session workspace is not a directory", 502);
  } catch (error) {
    if (error instanceof GitInspectionError) throw error;
    throw new GitInspectionError("The session workspace cannot be inspected", 502, (error as Error).message);
  }
  while (true) {
    try {
      ...
      if (code !== "ENOENT" && code !== "ENOTDIR") {
        throw new GitInspectionError("Repository metadata cannot be inspected", 502, (error as Error).message);
      }
    }
...
export interface GitInspectionLike {
  ...
  diff(cwd: string, pathId: string, side: GitDiffSide, signal?: AbortSignal): Promise<GitDiffResponse>;
}

export class GitInspectionService implements GitInspectionLike {
  constructor(private readonly runner: GitRunner = spawnGit) {}

  private async inspect(cwd: string, signal?: AbortSignal): Promise<InspectionRepository | null> {
    ...
    try {
      ...
      if (error instanceof GitInspectionError && error.message === "Git inspection failed" && error.exitCode === 128) {
    ...
    if (stripTerminatingLf(inside.stdout).toString("ascii") !== "true") {
      throw new GitInspectionError("Git returned malformed repository identity");
    }
    ...
    if (!repoRoot || !isAbsolute(repoRoot)) throw new GitInspectionError("Git returned malformed repository identity");
    ...
    const status = await this.runner(
      [
        ...GIT_CONFIG_ARGS, "-C", repoRoot, "status", "--porcelain=v2", "--branch", "-z",
        "--untracked-files=all", "--ignore-submodules=none", "--no-ahead-behind",
      ],
      { stdoutLimit: GIT_STATUS_OUTPUT_BYTES, signal },
    );
    return { ...parsePorcelainV2(status.stdout, Buffer.from(workspacePrefix)), repoRoot };
  }
  ...
  async diff(cwd: string, pathId: string, side: GitDiffSide, signal?: AbortSignal): Promise<GitDiffResponse> {
    if (pathId.length === 0 || pathId.length > MAX_PATH_ID_LENGTH || !/^[A-Za-z0-9_-]+$/.test(pathId)) {
      throw Object.assign(new Error("Invalid Git path identity"), { status: 400 });
    }
    const inspected = await this.inspect(cwd, signal);
    if (!inspected) throw Object.assign(new Error("That session is not in a Git repository"), { status: 409 });
    const change = inspected.response.files.find((file) => file.path.id === pathId);
    const authorized = change?.conflict ??
      (side === "staged" ? change?.staged : change?.unstaged ?? (change?.untracked ? { kind: "added" } : undefined));
    if (!change || !authorized) throw Object.assign(new Error("That path and diff side are not present in fresh status"), { status: 409 });
    const base = { path: change.path, side } as const;
    if (change.conflict) return { ...base, kind: "conflict", code: change.conflict.code };
    if (change.submodule) return { ...base, kind: "submodule", state: change.submodule };
    if (change.untracked) return { ...base, kind: "unsupported", reason: "untracked-content" };
    if (!change.path.utf8Path) return { ...base, kind: "unsupported", reason: "path-encoding" };

    const common = [...GIT_CONFIG_ARGS, "-C", inspected.repoRoot, "diff", "--no-ext-diff", "--no-textconv"];
    ...
    if (numstat.stdout.subarray(0, 4).toString("ascii") === "-\t-\t") return { ...base, kind: "binary" };
    const patch = await this.runner(
      [...common, "--unified=3", ...cached, "--", change.path.utf8Path],
      { stdoutLimit: GIT_DIFF_OUTPUT_BYTES, allowStdoutTruncation: true, signal },
    );
    if (patch.stdout.length === 0) return { ...base, kind: "empty", reason: "no-changes" };
    return { ...base, kind: "text", ...parseUnifiedDiff(patch.stdout, patch.truncated) };
  }
