...
const MAX_DEPTH = 4;
const REDACTED_KEY = /(?:authorization|cookie|token|password|secret|credential|prompt|message|content|payload|result|stderr|environment|(?:^|[_-])env(?:$|[_-])|headers|toolOutput|api[_-]?key|bearer)/i;
...
export function defaultDiagnosticLogPath(root: string, host: string, port: number): string {
  const stateHome = process.env.XDG_STATE_HOME || join(homedir(), ".local", "state");
  return join(stateHome, "inspire", "logs", `${installationKey(root, host, port)}.jsonl`);
}
...
async function verifyPrivateDirectory(path: string, create: boolean): Promise<void> {
  if (create) await mkdir(path, { recursive: true, mode: PRIVATE_DIRECTORY_MODE });
  const info = await lstat(path);
  if (!info.isDirectory() || info.isSymbolicLink()) {
    throw new Error(`The insπre diagnostics directory is not a private directory: ${path}`);
  }
  if (typeof process.getuid === "function" && info.uid !== process.getuid()) {
    throw new Error(`The insπre diagnostics directory is not owned by the current user: ${path}`);
  }
  if ((info.mode & 0o077) !== 0) {
    if (!create) throw new Error(`The insπre diagnostics directory must not be accessible by other users: ${path}`);
...
async function openPrivateAppend(path: string): Promise<FileHandle> {
  ...
  const handle = await open(path, constants.O_WRONLY | constants.O_CREAT | constants.O_APPEND | noFollow, PRIVATE_FILE_MODE);
  try {
    const info = await handle.stat();
    if (!info.isFile()) throw new Error(`The insπre diagnostics path is not a regular file: ${path}`);
    if (typeof process.getuid === "function" && info.uid !== process.getuid()) {
      throw new Error(`The insπre diagnostics file is not owned by the current user: ${path}`);
    }
...
export class FileDiagnosticLogger implements DiagnosticLogger {
  ...
  private constructor(private readonly path: string, options: FileDiagnosticLoggerOptions) {
    ...
    this.retainedFiles = Math.max(1, Math.min(20, options.retainedFiles ?? DEFAULT_RETAINED_FILES));
  }

  static async open(options: FileDiagnosticLoggerOptions): Promise<FileDiagnosticLogger> {
    await verifyPrivateDirectory(dirname(options.path), options.createPrivateDirectory ?? false);
  ...
  record(level: DiagnosticLevel, event: string, fields: Record<string, unknown> = {}): void {
    ...
    this.queue = this.queue.then(() => this.append(encoded)).catch((error) => {
      if (this.reportedFailure) return;
      this.reportedFailure = true;
      console.error("Unable to persist insπre diagnostics", error instanceof Error ? error.message : String(error));
    });
  }
...
export async function openDiagnosticLogger(options: FileDiagnosticLoggerOptions): Promise<DiagnosticLogger> {
  return FileDiagnosticLogger.open(options);
}

export function nullDiagnosticLogger(hostId = randomBytes(12).toString("base64url")): DiagnosticLogger {
  return {
    hostId,
    record: () => undefined,
