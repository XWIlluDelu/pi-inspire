...
function plainText(value: unknown, limit = BRANCH_SNIPPET_CHARS): string {
  ...
  const visit = (item: unknown, depth: number): void => {
    ...
    if (typeof item === "string") pieces.push(item);
      ...
      for (const child of item.slice(0, 64)) visit(child, depth + 1);
...
function entrySnippet(entry: SessionEntry): string {
  if (entry.type === "message")
    return plainText((entry.message as { content?: unknown }).content);
  if (entry.type === "custom_message") return plainText(entry.content);
  if (entry.type === "compaction" || entry.type === "branch_summary")
    return plainText(entry.summary);
  if (entry.type === "label")
    return entry.label?.slice(0, BRANCH_SNIPPET_CHARS) ?? "Label removed";
  if (entry.type === "session_info")
    return entry.name?.slice(0, BRANCH_SNIPPET_CHARS) ?? "Session information";
  if (entry.type === "model_change")
    return `${entry.provider}/${entry.modelId}`.slice(0, BRANCH_SNIPPET_CHARS);
...
export function activePathFor(
  ...
  while (id) {
    ...
    if (path.length > entries.length)
      throw new Error("Session tree depth is invalid");
  }
  return path.reverse();
}
...
export function projectSessionTree(
  ...
  for (const entry of entries) {
    const parentDepth =
      entry.parentId === null ? -1 : depthById.get(entry.parentId);
    if (entry.parentId !== null && parentDepth === undefined)
      throw new Error(
    ...
    if (entry.type === "label" && entry.label)
      labels.set(entry.targetId, entry.label.slice(0, 120));
  }
  ...
  const projectionBudget =
    BRANCH_TREE_MAX_BYTES - BRANCH_TREE_WRAPPER_RESERVE_BYTES;
  while (
    nodes.length > 1 &&
  ...
  if (
    nodes.length < projectedEntries.length ||
    activePath.length < completeActivePath.length
  )
    truncated = true;
  if (
    Buffer.byteLength(JSON.stringify({ nodes, activePath })) > projectionBudget
