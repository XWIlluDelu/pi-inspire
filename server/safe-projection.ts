...
export function projectSafeValue(
  value: unknown,
  ...
  if (Array.isArray(value)) {
    return value
      ...
      .map((item) => projectSafeValue(item, limits, depth + 1));
  }
  if (!value || typeof value !== "object") return value;
  const entries = Object.entries(value as Record<string, unknown>);
  const bounded =
    limits.objectEntries === undefined
      ? entries
      : entries.slice(0, limits.objectEntries);
  return Object.fromEntries(
    bounded.map(([key, child]) => [
      key,
      isSensitiveProjectionKey(key)
        ? REDACTED_VALUE
        : projectSafeValue(child, limits, depth + 1),
    ]),
  );
}
