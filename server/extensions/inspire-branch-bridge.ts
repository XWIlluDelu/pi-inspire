...
export { BRANCH_BRIDGE_MAX_ARGUMENT_BYTES, BRANCH_BRIDGE_MAX_RESULT_BYTES, BRANCH_BRIDGE_VERSION } from "../../shared/branch-bridge-protocol.js";
...
function parseRequest(argument: string, workerId: string): BranchBridgeRequest {
  const decoded = decodeBranchBridgeJson(argument, BRANCH_BRIDGE_MAX_ARGUMENT_BYTES);
  if (!decoded || typeof decoded !== "object" || Array.isArray(decoded)) throw new Error("invalid bridge request");
  const value = decoded as Record<string, unknown>;
  if (
    value.v !== BRANCH_BRIDGE_VERSION ||
    value.operation !== "navigate" ||
    typeof value.nonce !== "string" || !TOKEN.test(value.nonce) ||
    typeof value.workerId !== "string" || value.workerId !== workerId ||
    typeof value.sessionId !== "string" || !TOKEN.test(value.sessionId) ||
    typeof value.targetId !== "string" || !ENTRY_ID.test(value.targetId) ||
    Object.keys(value).some((key) => !["v", "nonce", "workerId", "sessionId", "operation", "targetId"].includes(key))
...
function emitResult(ctx: ExtensionCommandContext, statusKey: string, result: BranchBridgeResult): void {
  let text: string;
  try {
    ...
    } satisfies BranchBridgeResult, BRANCH_BRIDGE_MAX_RESULT_BYTES);
  }
...
export default function inspireBranchBridge(pi: ExtensionAPI): void {
  ...
  if (!TOKEN.test(command) || !TOKEN.test(statusKey) || !TOKEN.test(workerId)) return;
  ...
  pi.registerCommand(command, {
    ...
    handler: async (argument, ctx) => {
      ...
      try {
        ...
        if (ctx.mode !== "rpc" || !ctx.isIdle() || ctx.hasPendingMessages()) throw new Error("branch navigation requires an idle RPC session");
        const currentSessionId = ctx.sessionManager.getSessionId();
        if (currentSessionId !== request.sessionId) throw new Error("bridge request belongs to another session");
        if (!ctx.sessionManager.getEntry(request.targetId)) throw new Error("branch target does not exist");
        result.beforeLeaf = ctx.sessionManager.getLeafId();
        const navigation = await ctx.navigateTree(request.targetId, { summarize: false });
