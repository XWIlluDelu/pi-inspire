...
function projectPreferences(value: unknown): { preferences: InspirePreferences; warning?: string } {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {
      preferences: structuredClone(defaultPreferences),
      warning: "Saved preferences have an invalid root value. Repair or remove the file before changing settings.",
    };
  }

  const source = value as Record<string, unknown>;
  const normalized = structuredClone(defaultPreferences) as unknown as Record<string, unknown>;
  ...
  const unknown = Object.keys(source).filter((field) => !Object.hasOwn(preferenceFields, field));
  const issues = [...invalid, ...unknown.map((field) => `unknown:${field}`)];
  const issueSummary = issues.length <= 8
    ? issues.join(", ")
...
export class PreferencesStore {
  ...
  private async readDisk(): Promise<DiskPreferences> {
    ...
    try {
      ...
      if (error instanceof SyntaxError) {
        return {
          preferences: structuredClone(defaultPreferences),
          warning: "Saved preferences are not valid JSON. Repair or remove the file before changing settings.",
        };
      }
      throw error;
    }
  ...
  private invalidSourceError(current: DiskPreferences): Error | null {
    return current.warning
      ? Object.assign(new Error(`${current.warning} The saved file at ${this.path} was left unchanged.`), { status: 409 })
      : null;
  }
  ...
  async inspect(): Promise<PreferencesInspection> {
    return this.enqueue(async () => {
      ...
      return {
        ...
        ...(current.warning ? { warning: `${current.warning} The saved file at ${this.path} was left unchanged.` } : {}),
      };
    });
  }
  ...
  async patch(value: unknown): Promise<InspirePreferences> {
    ...
    return this.enqueue(async () => {
      ...
      const preferences = preferencesSchema.parse({ ...current.preferences, ...patch });
  ...
  async removeSession(sessionId: string): Promise<InspirePreferences> {
    return this.enqueue(async () => {
      ...
      const preferences = preferencesSchema.parse({
        ...current.preferences,
        pinnedSessionIds: current.preferences.pinnedSessionIds.filter((id) => id !== sessionId),
        hiddenSessionIds: current.preferences.hiddenSessionIds.filter((id) => id !== sessionId),
      });
      await this.persist(preferences);
      return preferences;
    });
